...
{
	#ifdef _WIN64
		SetWindowLongPtr(m_hEditWnd, GWLP_WNDPROC, (LONG_PTR)m_hOldProc);
	#else
...
{
		#ifdef _WIN64
			CUITextInputBox* pTextInputBox = (CUITextInputBox*)GetWindowLongPtrW(hWnd, GWLP_USERDATA);
		#else
...
void CUITextInputBox::Init(HWND hWnd, int iWidth, int iHeight, int iMaxLength, BOOL bIsPassword)
{
	...
	if (m_hEditWnd)
	{
		SetTextLimit(iMaxLength);
			#ifdef _WIN64
					m_hOldProc = (WNDPROC)SetWindowLongPtr(m_hEditWnd, GWLP_WNDPROC, (LONG_PTR)EditWndProc);
			#else
...
void CUITextInputBox::Render()
{
	...
	if (CheckOption(UIOPTION_PAINTBACK))
	{
		...
		RenderColor(m_iPos_x, m_iPos_y - 3, m_iWidth + 3, m_iHeight);
		EndRenderColor();
	}
