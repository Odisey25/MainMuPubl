...
bool CNewUITrade::Render()
{
	...
	{
		g_pTrade->SetPos(640 - 190 * 3 + DisplayWinCDepthBox, 0);
	}
	else
	{
		g_pTrade->SetPos(640 - 190 * 2 + DisplayWinCDepthBox, 0);
	}
...
void CNewUITrade::ProcessMyInvenCtrl()
{
	...
	if (SEASON3B::IsPress(VK_LBUTTON))
	{
		...
		if (pPickedItem->GetOwnerInventory() == g_pMyInventory->GetInventoryCtrl()) //Tu nhan vat sang o trade
		{
