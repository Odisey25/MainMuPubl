...
BOOL OpenInitFile()
{
	...
	mAutoHP = GetPrivateProfileInt("Custom", "mAutoHP", 0, szIniFilePath);
...
//#ifdef _DEBUG
	...
	if ( ERROR_SUCCESS == RegCreateKeyEx(HKEY_CURRENT_USER, "SOFTWARE\\Webzen\\Mu\\Config", 0, NULL, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, & hKey, &dwDisp))
	{
		...
		dwSize = 32;
		if (RegQueryValueEx(hKey, "PW", 0, NULL, (LPBYTE)m_Pass, &dwSize) != ERROR_SUCCESS)
		{
		}
		dwSize = sizeof(int);
...
char* szServerIpAddress = "192.168.56.1";
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int nCmdShow)
{
	...
	void CreateCustomFonts(int screenWidth, int m_FontSizePlus);
	{
