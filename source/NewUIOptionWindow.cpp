...
bool SEASON3B::CNewUIOptionWindow::UpdateMouseEvent()
{
	...
	if(CheckMouseIn(m_Pos.x+33 - 8, m_Pos.y+270, 124+8, 16))
	{
...
void SEASON3B::CNewUIOptionWindow::RenderFrame()
{
	...
	g_pBCustomMenuInfo->gInfoBox(m_Pos.x + 17.0f, m_Pos.y + 50.0f, 190, 40, 0x00000060, 0, 0);

	g_pBCustomMenuInfo->gInfoBox(m_Pos.x + 17.0f, m_Pos.y + 90.0f, 190, 95, 0x00000060, 0, 0);

	g_pBCustomMenuInfo->gInfoBox(m_Pos.x + 17.0f, m_Pos.y + 190.0f, 190, 60, 0x00000060, 0, 0);
...
void SEASON3B::CNewUIOptionWindow::RenderCustomFrame()
{
	...
	g_pRenderText->RenderText(x + 30, y + 5, "Đồ Hoạ");
	...
	if (g_pBCustomMenuInfo->RenderCheckBox(x + 15 + 80, y + 15, 0xFFCC00C8, this->OnOffGrap[eExcellentEffect] == 1 ? TRUE : FALSE, "Excellent Effect"))
	{
		this->OnOffGrap[eExcellentEffect] ^= 1;
	}*/

	y = y + 45;
	g_pRenderText->RenderText(x + 30, y, "Tuỳ Chọn");
	x = m_Pos.x + 25;
	if (g_pBCustomMenuInfo->RenderCheckBox(x, y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDWings] == 1 ? TRUE : FALSE, "Ẩn Cánh"))
	{
		this->OnOffGrap[eBMDWings] ^= 1;
	}
	if (g_pBCustomMenuInfo->RenderCheckBox(x + 30 + 80, y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDWeapons] == 1 ? TRUE : FALSE, "Ẩn Vũ Khí"))
	{
		this->OnOffGrap[eBMDWeapons] ^= 1;
	}
	y = y + 17;
	if (g_pBCustomMenuInfo->RenderCheckBox(x  , y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDImg] == 1 ? TRUE : FALSE, "Giảm Lag"))
	{
		this->OnOffGrap[eBMDImg] ^= 1;
	}
	if (g_pBCustomMenuInfo->RenderCheckBox(x + 30 + 80, y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDMonter] == 1 ? TRUE : FALSE, "Ẩn Quái"))
	{
		this->OnOffGrap[eBMDMonter] ^= 1;
	}
	y = y + 17;
	if (g_pBCustomMenuInfo->RenderCheckBox(x  , y + 15, 0xFFCC00C8, this->OnOffGrap[eRenderObjects] == 1 ? TRUE : FALSE, "Ẩn Địa Hình"))
	{
		this->OnOffGrap[eRenderObjects] ^= 1;
	}
	if (g_pBCustomMenuInfo->RenderCheckBox(x + 30 + 80, y + 15, 0xFFCC00C8, this->OnOffGrap[eRenderTerrain] == 1 ? TRUE : FALSE, "Ẩn Bản Đồ"))
	{
		this->OnOffGrap[eRenderTerrain] ^= 1;
	}
	y = y + 17;
	if (g_pBCustomMenuInfo->RenderCheckBox(x , y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDPlayer] == 1 ? TRUE : FALSE, "Ẩn Người Chơi"))
	{
		this->OnOffGrap[eBMDPlayer] ^= 1;
	}
	
	if (g_pBCustomMenuInfo->RenderCheckBox(x + 30 + 80, y + 15, 0xFFCC00C8, this->OnOffGrap[eBMDZen] == 1 ? TRUE : FALSE, "Ẩn Drop Zen"))
	{
	...
	g_pRenderText->RenderText(x+10, y, "Chiến Đấu");
...
void SEASON3B::CNewUIOptionWindow::RenderButtons()
{
	...
	//RenderImage(IMAGE_OPTION_VOLUME_BACK, m_Pos.x+40, m_Pos.y+270, 124.f, 16.f);

	//if(m_iVolumeLevel > 0)
	//{
	//	//RenderImage(IMAGE_OPTION_VOLUME_COLOR, m_Pos.x+ 40, m_Pos.y + 270, 124.f * 0.1f * 	(m_iVolumeLevel), 16.f);
	//}
