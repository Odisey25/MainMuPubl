...
bool SEASON3B::CNewUIHotKey::UpdateKeyEvent()
{
	...
	{
		if(g_MessageBox->IsEmpty() && !g_pNewUIMiniMap->Movement)
		{
	...
	else if (SEASON3B::IsPress(VK_TAB) == true)
	{
		...
		// Xử lý mở/tắt MiniMap nếu không ở Blood Castle
...
{
// Nếu đang ở Blood Castle, không cho phép mở cửa hàng
if (gMapManager.InBloodCastle() == true)
{
	return false; // Không thực hiện bất kỳ hành động nào
}
if (gMapManager.InChaosCastle() == true)
{
	return false; // Không thực hiện bất kỳ hành động nào
...
if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_INGAMESHOP) == false)
{
	...
	{
		SendRequestIGS_CashShopOpen(0); // Gửi yêu cầu mở CashShop
