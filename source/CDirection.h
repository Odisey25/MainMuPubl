...
public:
  ...
  CDirection();
  static CDirection &GetSingleton() {
    return Singleton<CDirection>::GetSingleton();
  }
  virtual ~CDirection();
  ...
  void SummonCreateMonster(int Type, int x, int y, float Angle,
                           bool NextCheck = true, bool SummonAni = true,
                           float AniSpeed = -1.0f);
  bool MoveCreatedMonster(int Type, int x, int y, float Angle, int Speed);
  bool ActionCreatedMonster(int Type, int Action, int Count,
                            bool TankerAttack = false, bool NextCheck = false);
};
