...
void CUIPhotoViewer::RenderPhotoCharacter()
{
	...
	if (c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6)
		glTranslatef(-o->Position[0], -o->Position[1], -o->Position[2] - 20.0f);
	else
...
#ifdef HIGHT_DEFINITION_DELTA_TIME
	Vector(0.95, 0.95, 0.95, o->Light);
#else
	Vector(1, 1, 1, o->Light);
#endif // !HIGHT_DEFINITION_DELTA_TIME
	...
	else if (gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6)
	{
		m_PhotoHelper.Position[2] += 20;
		m_PhotoHelper.Scale += 0.11;

	}

	RenderBug2(&m_PhotoHelper, TRUE);
	if (c->Helper.Type == MODEL_HELPER + 2)
		m_PhotoHelper.Position[2] -= 10;
	else if (c->Helper.Type == MODEL_HELPER + 3)
		m_PhotoHelper.Position[2] -= 25;
...
void CUIPhotoViewer::ClearItem()
{
	...
	memset(&m_PhotoHelper, 0, sizeof(m_PhotoHelper));
	m_PhotoChar.Helper.Type = -1;
	m_PhotoChar.Helper.Level = 0; //((Level>>3)&15);
	m_PhotoChar.Helper.ExtOption = 0;
	m_PhotoChar.Helper.Option1 = 0;


	m_PhotoChar.Flag.Type = 0x1FFF;
...
void CUIPhotoViewer::UpdateItemChar(int Slot, int IndexItem, int Level, int Ex, int Ancient)
{
	...
	case 8:
	{
		...
		//===Custom Pet
		if (gCustomPet2.CheckCustomPetByItem(m_PhotoChar.Helper.Type - MODEL_ITEM))
		{
	...
	case 238:
	{
		...
		m_PhotoChar.Flag.Type = IndexItem - MODEL_ITEM;
		//	ThePetProcess().CreatePet(MODEL_HELPER + 64 - 1171, MODEL_HELPER + 64, m_PhotoChar.Object.Position, &m_PhotoChar);
		//if (gCustomPet2.GetInfoPetType(IndexItem - MODEL_ITEM) == 8)
		//{
		//	CreateBugSub(IndexItem, m_PhotoChar.Object.Position, &m_PhotoChar.Object, &m_PhotoHelper);
		//
		//
		//}
	}
...
void CUIPhotoViewer::SetClass(BYTE byClass, int Flag)
{
	if (m_bIsInitialized == FALSE) return;
...
BOOL CUIFriendListTabWindow::HandleMessage()
{
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 2:
		{
			...
			//dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_OK, (GetWindowsX() / 2) - (100 / 2), 50, tempTxt, GetUIID());
			dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1017], GetUIID());
...
BOOL CUILetterBoxTabWindow::HandleMessage()
{
	...
	case UI_MESSAGE_BTNLCLICK:
	{
		...
		case 4:
		{
			...
			//dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, "123");
			dwUIID = g_pWindowMgr->AddWindow(UIWNDTYPE_OK, UIWND_DEFAULT, UIWND_DEFAULT, GlobalText[1017], GetUIID());
			//g_ConsoleDebug->Write(MCD_ERROR, "============== %d~ %d =================", m_WorkMessage.m_iMessage, m_WorkMessage.m_iParam1);
		}
		break;
		case 5:
		{
