...
void SetPlayerStop(CHARACTER* c)
{
	...
	{
		if ((c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5) && (!c->SafeZone || gProtect.m_MainInfo.HienPetTrongThanh == 1))
		{
		...
		else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && (!c->SafeZone || gProtect.m_MainInfo.HienPetTrongThanh == 1))
		{
			if (c->Weapon[0].Type == -1 && c->Weapon[1].Type == -1)
	...
	if (rand_fps_check(16))
	{
		...
		else if (c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5)
		{
			if (rand_fps_check(3))
			{
...
{
	if (gProtect.m_MainInfo.ChoPetChayTrongThanh == 1)
	{
		if (c->SafeZone && c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6 || c->Helper.Type == MODEL_HELPER + 4)
		{
			goto Jump;
		}
	}
...
Jump:
	...
	if (c->MonsterIndex >= 534 && c->MonsterIndex <= 539)
	{
		...
		if (!c->SafeZone && c->Run < 40)
		{
			if (gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK || gCharacterManager.GetBaseClass(c->Class) == CLASS_DARK_LORD
				...
				|| c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5
	...
	if (gProtect.m_MainInfo.ChoPetChayTrongThanh == 1)
	{
		if (c->SafeZone && c->Helper.Type == MODEL_HELPER + 37 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 5 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6 || c->Helper.Type == MODEL_HELPER + 4)
		{
			//g_ConsoleDebug->Write(MCD_SEND, "SauCheck"); // chay trong thanh
			c->Run++;
		}
	}
...
void SetAttackSpeed()
{
	...
	float RageAttackSpeed = CharacterAttribute->PrintPlayer.ViewPhysiSpeed * 0.002f;
...
bool AttackStage(CHARACTER* c, OBJECT* o)
{
	...
	case AT_SKILL_BLOOD_ATT_UP: // Fire_Slash
...
void MoveCharacter(CHARACTER* c, OBJECT* o) // Chỉnh Size Nhân Vật Ở Đây
{
	if (o->Type == MODEL_WARCRAFT)
	{
		char Text[100];
	...
	if ((int)c->AttackTime >= g_iLimitAttackTime)
	{
		...
		case AT_SKILL_WHEEL:
		case MASTER_SKILL_ADD_TWISTING_SLASH_IMPROVED1: // Xoay Kiếm / DK 
		...
		case AT_SKILL_BLAST_HELL:
			...
			break;
		case AT_SKILL_BLAST_POISON: //Mưa Độc Tố DW // Dộc DW
		case MASTER_SKILL_ADD_DECAY_IMPROVED:
		{
			vec3_t Position, Pos;
			Position[0] = (float)(c->SkillX + 2.2f) * TERRAIN_SCALE;
			...
			for (int i = 0; i < 5; ++i)  // Số lượng hạt độc rơi xuống
			{
				...
				CreateEffect(MODEL_FIRE, Pos, o->Angle, Light, 6, NULL, i + 1);
			}
		...
		case AT_SKILL_ICE_UP: //Mưa Sao Băng DW // Dộc DW
		...
		case AT_SKILL_BLAST_FREEZE:
		{
			...
			TargetPosition[0] = (float)(c->SkillX + 1.5f) * TERRAIN_SCALE;
		...
		case AT_SKILL_ALICE_ENERVATION:
			...
			break;
		case AT_SKILL_FLAME_STRIKE: // Flame Strike
		{
		...
		else
		{
			...
			case MASTER_SKILL_ADD_FORCE_WAVE_IMPROVED:
				...
				CreateEffect(MODEL_WAVES, vec3_t{ o->Position[0], o->Position[1], o->Position[2] + 100.0f }, o->Angle, o->Light, 1);
				CreateEffect(MODEL_WAVES, vec3_t{ o->Position[0], o->Position[1], o->Position[2] + 100.0f }, o->Angle, o->Light, 1);
				CreateEffect(MODEL_PIERCING2, vec3_t{ o->Position[0], o->Position[1], o->Position[2] + 100.0f }, o->Angle, o->Light);
			...
			break;
			case AT_SKILL_ALICE_DRAINLIFE_UP: // Hút Máu SM
...
float CharacterMoveSpeed(CHARACTER* c)
{
	...
	if (o->Type == MODEL_PLAYER && o->Kind == KIND_PLAYER)
	{
		...
		else if ((c->Helper.Type == MODEL_HELPER + 4 || gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) == 6) && !c->SafeZone && !isholyitem)
		{
			c->Run = 40;
			Speed = 17;
		}
...
void RenderLinkObject(float x, float y, float z, CHARACTER* c, PART_t* f, int Type, int Level, int Option1, bool Link, bool Translate, int RenderType, bool bRightHandItem)
{
	...
	if (Type != MODEL_WING + 7857 // Cape of Lord
		&& Type != MODEL_WING + 7364 // Cape of Fighter
		&& Type != MODEL_WING + 130 // Small Cape of Lord
		&& Type != MODEL_WING + 135 // Small Cape of Fighter
		)
	{
		RenderPartObjectEffect(Object, Type, c->Light, o->Alpha, Level, Option1, false, 0, RenderType | ((c->MonsterIndex == 67 || c->MonsterIndex == 137) ? (RENDER_EXTRA | RENDER_TEXTURE) : RENDER_TEXTURE));
	}
	...
	for (std::map<int, DYNAMIC_WING_EFFECT_INFO>::iterator it = gDynamicWingEffect.m_DynamicWingEffectInfo.begin(); it != gDynamicWingEffect.m_DynamicWingEffectInfo.end(); it++)
	{
		if (it->second.ItemIndex == (Type - MODEL_ITEM))
		{
			...
			//g_ConsoleDebug->Write(1, "EFFECT IndexItem %d ~ itemtype %d", it->second.ItemIndex, (Type - MODEL_ITEM));
		}
	...
	for (std::map<int, CUSTOM_WING_EFFECT_INFO>::iterator it2 = gCustomWingEffect.m_CustomWingEffectInfo.begin(); it2 != gCustomWingEffect.m_CustomWingEffectInfo.end(); it2++)
	{
		if (it2->second.ItemIndex == (Type - MODEL_ITEM))
		{
			...
			//CreateParticle(BITMAP_SPARK + 1, Position, o->Angle, Light, 11, 2.0f);
		//	g_ConsoleDebug->Write(1, "EFFECT IndexItem %d ~ itemtype %d", it2->second.ItemIndex, (Type - MODEL_ITEM));
		}
	}
	switch (Type)
	{
...
void RenderCharacter(CHARACTER* c, OBJECT* o, int Select)
{
	...
	if (!Bind)
	{
		if (!g_isCharacterBuff(o, eBuff_Cloaking))
		{
			...
			if (g_isCharacterBuff(o, eBuff_AddCriticalDamage) && o->Kind == KIND_PLAYER && o->Type == MODEL_PLAYER && (c->LastCritDamageEffect < WorldTime - CritDamageEffectInterval))
			{
				...
				if (c->Weapon[0].Type != -1 && c->Weapon[0].Type != MODEL_ARROWS)//&& ( c->Weapon[0].Type<MODEL_SHIELD || c->Weapon[0].Type>=MODEL_SHIELD+MAX_ITEM_INDEX ) )
				{
					...
					CreateEffectFpsChecked(BITMAP_FLARE_FORCE, Position, o->Angle, o->Light, 1, o, weaponType, c->Weapon[0].LinkBone);
					if (renderSkillWave == false)
					{
						CreateEffectFpsChecked(MODEL_DARKLORD_SKILL, Position, o->Angle, Light, 0);
					}
				}
				if (c->Weapon[1].Type != -1 && c->Weapon[1].Type != MODEL_BOLT && (c->Weapon[1].Type < MODEL_SHIELD || c->Weapon[1].Type >= MODEL_SHIELD + MAX_ITEM_INDEX))
				{
					...
					CreateEffectFpsChecked(BITMAP_FLARE_FORCE, Position, o->Angle, o->Light, 1, o, weaponType, c->Weapon[1].LinkBone);
					if (renderSkillWave == false)
					{
						CreateEffectFpsChecked(MODEL_DARKLORD_SKILL, Position, o->Angle, Light, 1);
					}
				}
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (w->Type != -1 && w->Type != MODEL_BOW + 7 && w->Type != MODEL_BOW + 15 && w->Type != MODEL_HELPER + 5)//CustomArrow Cung,No
			{
				...
				else if (w->Type == MODEL_PHOENIX_SOUL_STAR) { // Hiệu Ứng Vuốt Phượng Hoàng
	...
	case MODEL_PLAYER:
		...
		// ChainLighting
		else if (o->CurrentAction == PLAYER_SKILL_CHAIN_LIGHTNING) // Lôi Điện SM
        {
           vec3_t vRelativePos, vWorldPos, vLight;
           Vector(0.f, 0.f, 0.f, vRelativePos);
           Vector(0.4f, 0.4f, 0.8f, vLight);
        
        // Hiệu ứng sét ở tay trái
        b->TransformPosition(o->BoneTransform[37], vRelativePos, vWorldPos, true); // "Bip01 L Hand"
        
        
        for (int i = 0; i < 5; ++i) // Tăng số lượng sét
        {
        	CreateEffect(MODEL_FENRIR_THUNDER, vWorldPos, o->Angle, vLight, 2, o);
        }
        
        // Tăng hiệu ứng sáng xung quanh
        CreateSprite(BITMAP_LIGHT, vWorldPos, 2.5f, vLight, o, (float)(rand() % 360));
        CreateSprite(BITMAP_LIGHT, vWorldPos, 2.0f, vLight, o, (float)(rand() % 360));
        CreateSprite(BITMAP_LIGHT, vWorldPos, 1.5f, vLight, o, (float)(rand() % 360));
        
        // Hiệu ứng sét ở tay phải
		...
		if (g_isCharacterBuff((&c->Object), eBuff_Berserker)) // Buff Hộ Công
		{
			static bool isEffectActive = false; // Cờ kiểm tra xem hiệu ứng đã được tạo chưa

			if (!isEffectActive) // Chỉ tạo hiệu ứng khi chưa có
			{
				vec3_t vLight[2];
				...
				for (int i = 0; i < 4; ++i)
				{
					if (!SearchEffect(BITMAP_ORORA, o, i)) // Kiểm tra từng hiệu ứng trước khi tạo
						CreateEffect(BITMAP_ORORA, o->Position, o->Angle, vLight[0], i, o);

					if ((i == 2 || i == 3) && !SearchEffect(BITMAP_SPARK + 2, o, i))
						CreateEffect(BITMAP_SPARK + 2, o->Position, o->Angle, vLight[1], i, o);
				}

				if (!SearchEffect(BITMAP_LIGHT_MARKS, o, 0))
					CreateEffect(BITMAP_LIGHT_MARKS, o->Position, o->Angle, vLight[0], 0, o);

				isEffectActive = true; // Đánh dấu là đã tạo hiệu ứng
			}
		}
		else
		{
			static bool isEffectActive = true; // Cờ kiểm tra xem hiệu ứng đã xóa chưa

			if (isEffectActive) // Chỉ xóa hiệu ứng nếu nó đang tồn tại
			{
				for (int i = 0; i < 4; ++i)
				{
				...
				isEffectActive = false; // Đánh dấu là đã xóa hiệu ứng
			}
		}
...
void ChangeCharacterExt(int Key, BYTE* Equipment, CHARACTER* pCharacter, OBJECT* pHelper)
{
	...
	if (lpCharSet > 0)
	{
		//g_ConsoleDebug->Write(1, "NEW_PET_STRUCT Key %d", c->Key);
