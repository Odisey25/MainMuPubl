...
void SEASON3B::CNewUIInventoryCtrl::RenderNumberOfItem()
{
	...
	for (; li != m_vecItem.end(); ++li)
	{
		...
		else if (gCustomItemStack.IsStackItem(pItem->Type, (pItem->Level >> 3) & 15) == 1 && pItem->Durability > 1)
		{
			glColor3f(1.f, 0.9f, 0.7f);
			SEASON3B::RenderNumber(x + width - 6, y + 1, pItem->Durability);
		}
...
bool SEASON3B::CNewUIInventoryCtrl::AreItemsStackable(ITEM* pSourceItem, ITEM* pTargetItem)
{
	...
	if (gCustomItemStack.IsStackItem(iSrcType, (iSrcLevel >> 3) & 15) == 1 && gCustomItemStack.IsStackItem(iTarType, (iTarLevel >> 3) & 15) == 1 && (iSrcDurability < 255 && iTarDurability < 255))
	{
		return true;
	}
...
bool IsRingPen(int Index)
{
	if ((Index >= 6664 && Index <= 6665) ||
		(Index >= 6668 && Index <= 6669) ||
		(Index >= 6677 && Index <= 6680) ||
		(Index >= 6681 && Index <= 6684))
	{
		return true;
	}
	return false;
