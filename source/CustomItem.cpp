...
BOOL CCustomItem::GetCustomItemColor(int ItemIndex,float* ItemColor) // OK
{
	...
	std::map<int, CUSTOM_RING_INFO>::iterator it3 = gCustomRingPen.m_CustomRingPenInfo.find(ItemIndex);
	...
	else if (it3 != gCustomRingPen.m_CustomRingPenInfo.end())
	{
		ItemColor[0] = (float)(it3->second.ColorR / 255.0f);
		ItemColor[1] = (float)(it3->second.ColorG / 255.0f);
		ItemColor[2] = (float)(it3->second.ColorB / 255.0f);
		return 1;
	}
	else
	{
