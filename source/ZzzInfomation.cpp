...
void ItemConvert(ITEM *ip,BYTE Attribute1,BYTE Attribute2, BYTE Attribute3 )
{
		...
		)
    {
	 ...
	 if (gCustomWing.CheckCustomWingByItem(ip->Type)) //Custom Wing Opt Exc
	{
		 
		if (excelWing & 0x01)
		{
			g_ConsoleDebug->Write(MCD_ERROR, "Type %d ~ isCustomWing %d", gCustomWing.GetCustomWingNewOptionValue(ip->Type, 0), gCustomWing.GetCustomWingNewOptionIndex(ip->Type, 0));
	...
	if(Option3)
	{
        ...
        else
        {
		    ...
		    if(ip->Type>=ITEM_HELPER+8 && ip->Type<ITEM_HELPER+MAX_ITEM_INDEX && ip->Type!=ITEM_HELPER+30 || gCustomRingPen.CheckCustomRing(ip->Type) == 1 || ip->Type == ITEM_HELPER +180) //saucheck
		    {
	...
	// them opt exl ring
	if((ip->Type>=ITEM_SHIELD && ip->Type<ITEM_BOOTS+MAX_ITEM_INDEX) || (ip->Type>=ITEM_HELPER+8 && ip->Type<=ITEM_HELPER+9) || (ip->Type>=ITEM_HELPER+21 && ip->Type<=ITEM_HELPER+24)|| gCustomRingPen.CheckCustomRing(ip->Type))
	{
...
//	g_ConsoleDebug->Write(MCD_ERROR, "Check %d , type %d", gCustomRingPen.CheckCustomPen(ip->Type), ip->Type);
	if((ip->Type>=ITEM_SWORD && ip->Type<ITEM_STAFF+MAX_ITEM_INDEX) || (ip->Type>=ITEM_HELPER+12 && ip->Type<=ITEM_HELPER+13) || (ip->Type>=ITEM_HELPER+25 && ip->Type<=ITEM_HELPER+28) || gCustomRingPen.CheckCustomPen(ip->Type))
	{
		if((Attribute2>>5)&1)
		{
		...
		if ( (ip->Type>=ITEM_STAFF && ip->Type<ITEM_STAFF+MAX_ITEM_INDEX) ||
           ...
           || ( ip->Type==ITEM_HELPER+25 || ip->Type==ITEM_HELPER+27 ) || gCustomRingPen.CheckCustomRingPen(ip->Type) == 3) 
		{
			if((Attribute2>>4)&1)
			{
    ...
    else if((ip->Type>=ITEM_HELPER+8 && ip->Type<ITEM_HELPER+12)||(ip->Type == ITEM_HELPER+20 && Level == 0) // saucheck
...
void CHARACTER_MACHINE::CalculateAttackSpeed()
{
	...
	{
		Character.AttackSpeed = Dexterity / 200;  // Tăng giá trị chia từ 50 lên 200 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 200;
	}
	else if (CharacterClass == CLASS_KNIGHT || CharacterClass == CLASS_DARK)
	{
		Character.AttackSpeed = Dexterity / 60;  // Tăng giá trị chia từ 15 lên 60 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 80;   // Tăng giá trị chia từ 20 lên 80 (giảm tốc độ đánh)
	}
	else if (CharacterClass == CLASS_DARK_LORD)
	{
		Character.AttackSpeed = Dexterity / 40;  // Tăng giá trị chia từ 10 lên 40 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 40;
	}
	else if (CharacterClass == CLASS_SUMMONER)
	{
		SetAttackSpeed();
		Character.AttackSpeed = Dexterity / 80;  // Tăng giá trị chia từ 20 lên 80 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 80;
	}
	else if (CharacterClass == CLASS_RAGEFIGHTER)
	{
		Character.AttackSpeed = Dexterity / 36;  // Tăng giá trị chia từ 9 lên 36 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 36;
	}
	else
	{
		Character.AttackSpeed = Dexterity / 80;  // Tăng giá trị chia từ 20 lên 80 (giảm tốc độ đánh)
		Character.MagicSpeed = Dexterity / 40;   // Tăng giá trị chia từ 10 lên 40 (giảm tốc độ đánh)
	}
	...
	if (Right && Left)
	{
		Character.AttackSpeed += (RightWeaponSpeed + LeftWeaponSpeed) / 8;  // Giảm tốc độ đánh từ trang bị
		Character.MagicSpeed += (RightWeaponSpeed + LeftWeaponSpeed) / 8;
	}
	else if (Right)
	{
		Character.AttackSpeed += RightWeaponSpeed / 4;  // Giảm tốc độ đánh từ trang bị
		Character.MagicSpeed += RightWeaponSpeed / 4;
	}
	else if (Left)
	{
		Character.AttackSpeed += LeftWeaponSpeed / 4;  // Giảm tốc độ đánh từ trang bị
		Character.MagicSpeed += LeftWeaponSpeed / 4;
	}

	if (RingRight)
	{
		Character.AttackSpeed += 2;  // Giảm từ 10 xuống 2
		Character.MagicSpeed += 2;
	...
	if (Glove)
	{
		Character.AttackSpeed += g->WeaponSpeed / 4;  // Giảm tốc độ đánh từ găng tay
		Character.MagicSpeed += g->WeaponSpeed / 4;
	}

	if (Helper)
	{
		const ITEM_ADD_OPTION& Item_data = g_pItemAddOptioninfo->GetItemAddOtioninfo(ITEM_HELPER + 64);
		Character.AttackSpeed += Item_data.m_byValue2 / 4;  // Giảm giá trị cộng thêm
		Character.MagicSpeed += Item_data.m_byValue2 / 4;
	}
	...
	{
		Character.AttackSpeed += 5;  // Giảm từ 20 xuống 5
		Character.MagicSpeed += 5;
	}
	...
	if (g_isCharacterBuff((&Hero->Object), eBuff_EliteScroll1))
	{
		...
		Character.AttackSpeed += Item_data.m_byValue1 / 4;  // Giảm giá trị cộng thêm
		Character.MagicSpeed += Item_data.m_byValue1 / 4;
	}
	...
	{
		int _Temp = 3;  // Giảm từ 15 xuống 3
		Character.AttackSpeed += _Temp;
		Character.MagicSpeed += _Temp;
	}
	...
	Character.AttackSpeed += g_SocketItemMgr.m_StatusBonus.m_iAttackSpeedBonus / 4;  // Giảm giá trị cộng thêm
	Character.MagicSpeed += g_SocketItemMgr.m_StatusBonus.m_iAttackSpeedBonus / 4;
}
