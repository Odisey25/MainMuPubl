...
bool SEASON3B::CNewUIMiniMap::Render()
{
	...
	if (!gBAiFindPath.pathTimDuongDi.empty())
	{
		...
		for (int n = 0; n < gBAiFindPath.pathTimDuongDi.size(); n++)
		{
			...
			gInterface.DrawBarForm((x - 1 + (BplayerX * (m_MapWidth.x / 256.f))), ((y - 1) + (BplayerY * (m_MapWidth.y / 256.f)) - 5.0), 2, 2, 1.0, 0.0, 0.0, 0.75);

		}

		XNPC = (float)(x + (gBAiFindPath.ViTriCanDenX * (m_MapWidth.x / 256.f)));
		YNPC = (float)(y + ((255 - gBAiFindPath.ViTriCanDenY) * (m_MapWidth.y / 256.f)));
	...
	if (MouseX > m_MapPos.x && MouseY > m_MapPos.y && MouseX < m_MapPos.x + m_MapWidth.x && MouseY < m_MapPos.y + m_MapWidth.y)
	{
		...
		{
			{
				gBAiFindPath.TimDuongDi(g_pHeroPositionInfo->m_CurHeroPosition.x, g_pHeroPositionInfo->m_CurHeroPosition.y, ViTriChon.x, ViTriChon.y);
