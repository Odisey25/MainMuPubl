...
bool CNewUIMyInventory::TryConsumeItem(CNewUIInventoryCtrl* targetControl, ITEM* pItem, const int iIndex)
{
	...
	if ((pItem->Type >= ITEM_ETC + 0 && pItem->Type < ITEM_ETC + MAX_ITEM_INDEX)
		...
		|| (gCustomItemStack.IsStackItem(pItem->Type, (pItem->Level >> 3) & 15))
			...
			if (!EquipmentItem)
			{
				...
				LABEL_32: // wtf...
					if (g_pMyInventory->IsEquipable(nDstIndex, pItem))
					{
						...
						//SendRequestEquipmentItem((int)STORAGE_TYPE::INVENTORY, iSrcIndex, pItem, (int)STORAGE_TYPE::INVENTORY, nDstIndex);
					}
				}
...
{
	for (int i = ITEM_POTION; i < ITEM_POTION + 3; i++)
	{
...
BOOL SEASON3B::CNewUIMyInventory::IsInvenItem(const short sType)
{
	...
	if (FALSE
		...
		bInvenItem = TRUE;
	//g_ConsoleDebug->Write(MCD_RECEIVE, "[IsInvenItem] [%d] %d", sType, bInvenItem);
	return bInvenItem;
}
#endif //LJH_ADD_SYSTEM_OF_EQUIPPING_ITEM_FROM_INVENTORY
...
void SEASON3B::CNewUIMyInventory::BMoveItemNew(int Type)
{
	...
	if (Type == MOVE_INV_TO_MIX || Type == MOVE_INV_TO_CHAR) pMyInvenCtrl = g_pMyInventory->GetInventoryCtrl();
	else if (Type == MOVE_MIX_TO_INV)  pMyInvenCtrl = g_pMixInventory->GetInventoryCtrl();
	...
	if (pItemObj)
	{
		...
		{
			pMsg.sFlag = MOVE_INV_TO_MIX == Type ? 0 : 3;
			pMsg.tFlag = MOVE_INV_TO_MIX == Type ? 3 : 0;
			pMsg.Target = 0;
