...
bool IsBug(ITEM* pItem)
{
    ...
    if (gCustomPet2.CheckCustomPetByItem(pItem->Type) && gCustomPet2.GetInfoPetType(pItem->Type) != 8)
        return true;
...
void CreateBug(int Type, vec3_t Position, OBJECT* Owner, int SubType, int LinkBone)
{
    ...
    {
        OBJECT* o = &Butterfles[i];
        if (CreateBugSub(Type, Position, Owner, o, SubType, LinkBone) == TRUE)
            return;
    }
}
...
bool MoveBug(OBJECT* o, bool bForceRender)
{
    if (o->Live)
    {
        ...
        case MODEL_FENRIR_GOLD:
        GetFerrir:
            if (gProtect.m_MainInfo.HienPetTrongThanh == 0)
            {
        ...
        if (gCustomPet2.GetInfoPetType(o->Type - 1171) == 8)
        {
            ...
            if (bOwnerStopState || bOwnerMovingState)
            {
                ...
                if (o->Owner && !g_isCharacterBuff(o->Owner, eBuff_Cloaking))
                {
                    ...
                    bool bOwnerStanding = bOwnerStopState;
                    bool bOwnerMoving = !bOwnerStanding;
        ...
        if (o->Type == MODEL_HELPER || o->Type == MODEL_HELPER + 1 ||
            gCustomPet2.GetInfoPetType(o->Type - 1171) == 0 ||
            gCustomPet2.GetInfoPetType(o->Type - 1171) == 10 ||
            gCustomPet2.GetInfoPetType(o->Type - 1171) == 11)
        {
            vec3_t Range;
            VectorSubtract(TargetPosition, o->Position, Range);
            float Distance = Range[0] * Range[0] + Range[1] * Range[1];
...
bool RenderBug(OBJECT* o, bool bForceRender)
{
    if (o->Live)
    {
        ...
        if (o->Visible)
        {
            ...
            if (gCustomPet2.GetInfoPetType(o->Type - 1171) == 4 && SceneFlag == CHARACTER_SCENE)
            {
                o->Scale = 1.0f;
            }

            if (gCustomPet2.CheckCustomPetByItem(o->Type - 1171) == 1 && SceneFlag == 5)
            {
                o->Scale = gCustomPet2.GetScaleView(o->Type - 1171);
            }
            ...
            // 1) Find the owner CHARACTER of this pet
            ...
            // 2) Determine "shadow candidate" type
            ...
            // 3) Safe zone rules:
            // - Custom pets: can have shadow inside and outside safe zone
            // - MODEL_HELPER+2 / +3: no shadow in safe zone
            ...
            // 4) Final decision on shadow rendering
            bool needShadow = false;

            if (isCustomPet)
                needShadow = true; // Custom pets always allow shadow (subject to map restrictions)

            if ((isDefaultHelper2 || isDefaultHelper3) && !blockShadowInSafeZone)
                needShadow = true; // Default Helper2/3, only allow shadow outside safe zone

            if (needShadow)
            {
                // Map restrictions: consistent with character shadow rules
                if (gMapManager.WorldActive != WD_10HEAVEN
                    ...
                    && !g_Direction.m_CKanturu.IsMayaScene())
                {
                    ...
                    // Shadow sticks to ground height
                    ShadowPos[2] = RequestTerrainHeight(ShadowPos[0], ShadowPos[1]);

                    // Place model's BodyOrigin at ground position for RenderBodyShadow projection
                    VectorCopy(ShadowPos, b->BodyOrigin);

                    // Enable alpha test, draw semi-transparent black circle
                    EnableAlphaTest();
                    glColor4f(0.f, 0.f, 0.f, 0.3f * o->Alpha);   // Adjust transparency as needed

                    // Render shadow using model's own outline
                    b->RenderBodyShadow();

                    // Restore full white color to avoid affecting subsequent rendering
...
bool RenderBug2(OBJECT* o, bool bForceRender)
{
    if (o->Live)
    {
        o->Visible = (bForceRender == FALSE ? TestFrustrum2D(o->Position[0] * 0.01f, o->Position[1] * 0.01f, -20.f) : true);
        if (o->Visible)
        {
            ...
            //g_ConsoleDebug->Write(MCD_ERROR, "Loadtexture %d [%.0f]", gCustomPet2.GetInfoPetType(o->Type - 1171), gCustomPet2.GetScaleView(o->Type - 1171));
...
void RenderDarkHorseSkill(OBJECT* o, BMD* b)
{
    ...
    if (o->AnimationFrame >= 8.f && o->AnimationFrame <= 9.5f)
    {
        ...
        {
            float Matrix2[3][4];
            vec3_t Angle2, p2, Position2;

            Vector(0.f, 150.f * (o->WeaponLevel / 2) * FPS_ANIMATION_FACTOR, 0.f, p2);
            Vector(0.f, 0.f, (float)(rand() % 360), Angle2);

            for (int i = 0; i < 6; ++i)
            {
                Angle2[2] += 60.f;
                AngleMatrix(Angle2, Matrix2);
                VectorRotate(p2, Matrix2, Position2);
                VectorAdd(o->Position, Position2, Position2);
                CreateEffect(MODEL_GROUND_STONE + rand() % 2, Position2, o->Angle, o->Light);
            }
...
void MoveTornado(OBJECT* o)
{
    ...
    o->Angle[2] += 30.0f * FPS_ANIMATION_FACTOR;
    o->Scale = 1.0f;
    ...
    for (int i = 0; i < MAX_BOIDS; i++)
    {
        ...
        if (o->Live)
        {
            ...
            else
            {
                ...
                float dx2 = o->Position[0] - Hero->Object.Position[0];
                float dy2 = o->Position[1] - Hero->Object.Position[1];
                float Range2 = sqrtf(dx2 * dx2 + dy2 * dy2);
