...
void RenderTipTextList(const int sx, const int sy, int TextNum, int Tab, int iSort, int iRenderPoint, BOOL bUseBG, BOOL Render3DItem) // đây
{
	SIZE TextSize = {0, 0};
	...
	if (bUseBG == TRUE && TextNum > 0)
	{
		...
		g_pBCustomMenuInfo->RenderToolTipItem((float)iPos_x - 7, (fsy - FixHRender3DItem) - 10, (float)fWidth + 15, (float)fHeight + 20, 0x00000060, 0, 0);
...
bool pSetTooltipValueText(std::wstring* new_text, char* Format, int* line, int* color, int type, ITEM* item, ITEM_ATTRIBUTE* item_attr, int level)
{
	char Buffer[1024] = { 0 };
	switch (type)
	{
	case 0:
	{
		if (item->DamageMin)
		{
			...
			if (minindex + DamageMin < maxindex + DamageMax)
				sprintf(Buffer, Format, DamageMin + minindex, maxindex + DamageMax);
			else
				sprintf(Buffer, Format, maxindex + DamageMax, maxindex + DamageMax);
			if (DamageMin > 0)
			{
			...
			sprintf(Buffer, Format, DamageMin, DamageMax);
		}
	}
	break;
	case 1:
		sprintf(Buffer, Format, item->Durability);
		break;
	case 2:
	{
		int maxdefense = 0;
		...
		sprintf(Buffer, Format, item->Defense + maxdefense);
	...
	break;
	case 3:
		sprintf(Buffer, Format, item->MagicDefense);
		break;
	case 4:
		{
		int maxDurability = calcMaxDurability(item, item_attr, level);
		sprintf(Buffer, Format, item->Durability, maxDurability);
		}
		break;
	case 5:
		{
			sprintf(Buffer, Format, item->RequireLevel);
		}
		break;

	case 6:
		{
			sprintf(Buffer, Format, item->RequireStrength - si_iNeedStrength);

		}
		break;
	case 7:
		{
			sprintf(Buffer, Format, item->RequireDexterity - si_iNeedDex);
		}
		break;
	case 8:
		sprintf(Buffer, Format, item->RequireVitality);
		break;
	case 9:
		sprintf(Buffer, Format, item->RequireEnergy);
		break;
	case 10:
		sprintf(Buffer, Format, item->RequireCharisma);
		break;
	case 11:
		//sprintf(Buffer, Format, item->MagicPower);
		sprintf(TextList[TextNum], Format, item->MagicPower);
	...
	case 12:
		...
		break;
	case 14:
		sprintf(Buffer, Format, level + 1);
		break;
	case 15:
		sprintf(Buffer, Format, item->SuccessfulBlocking);
		if ((item->Option1 & 0x3F) != 0)
			*color = 1;
		break;
	case 16:
		sprintf(Buffer, Format, 10 * item->Durability);
		break;
	case 101:
		sprintf(Buffer, Format, item_attr->WeaponSpeed);
		break;
	case 102:
		sprintf(Buffer, Format, item_attr->WeaponSpeed);
		break;
	case 201:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, item->RequireLevel - CharacterAttribute->Level);
		}
		else
		{
			return 0;
	...
	case 202:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, (item->RequireStrength - Strength) - si_iNeedStrength);
		}
		else
		{
			return 0;
	...
	case 203:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, (item->RequireDexterity - Dexterity) - si_iNeedDex);
		}
		else
		{
			return 0;
	...
	case 204:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, item->RequireVitality - Vitality);
		}
		else
		{
			return 0;
	...
	case 205:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, item->RequireEnergy - Energy);
		}
		else
		{
			return 0;
	...
	case 206:
	{
		...
		{
			*color = 2;
			sprintf(Buffer, Format, item->RequireCharisma - Charisma);
		}
		else
		{
			return 0;
	...
	break;
	case 207:
		sprintf(Buffer, Format, level + 32);
		break;
	case 208:
		{
			//==WingSetIncDamage
			...
			{
				sprintf(Buffer, Format, gCustomWing.GetCustomWingIncDamage(item->Type, level));
			}
			else
			{
				sprintf(Buffer, Format, 2 * level + 39);
			}
		}
		
		break;
	case 209:
		sprintf(Buffer, Format, 2 * level + 20);
		break;
	case 210:
		sprintf(Buffer, Format, 2 * level + 12);
		break;
	case 211:
		sprintf(Buffer, Format, 2 * level + 25);
		break;
	case 212:
		sprintf(Buffer, Format, 2 * level + 24);
		break;
	case 213:
		sprintf(Buffer, Format, 2 * level + 10);
		break;
	case 214:
		sprintf(Buffer, Format, level + 10);
		break;
	case 216: //thuoc tinh
		{
	...
	if(strlen(Buffer)>1)wsprintf(Format, Buffer);
	return 1;
}
...
void RenderItemInfo(int sx, int sy, ITEM* ip, bool Sell, int Inventype, bool bItemTextListBoxUse, bool Render3DItem)
{
	...
	else if (field_11BB64 == -1) //đây nè
	{
		if (item_level)
			sprintf(TextList[TextNum], " %s +%d", TextName, item_level);
		else
			sprintf(TextList[TextNum], "s %s", TextName);
	}
	else
	{
		sprintf(TextList[TextNum], "%s", tooltip_level->second.name);
		...
		else
		{
				...
				for (int i = 0; i < 12; i++)
				{
					...
					else
					{
						...
						if (text_type != -1)
						{
							...
							if (!pSetTooltipValueText(&new_, (char*)text_.c_str(), &TextNum, &text_color, text_type, ip, item, item_level))
							{
								continue;
							}
							v146 = 1;
			...
			else
			{
				for (int i = 0; i < 8; ++i)
				{
					...
					if (text_type != -1)
					{
						if (!pSetTooltipValueText(&test__, (char*)text_.c_str(), &TextNum, &text_color, text_type, ip, item, item_level))
						{
							continue;
						}
...
void RenderItemName(int i,OBJECT *o,int ItemLevel,int ItemOption,int ItemExtOption,bool Sort) //Drop Ở Đây
{
	char Name[80];
...
{	
	...
	if (TypePosition > 0)
	{
		...
		case 5:
		{
			...
			Vector(270.0f, gCustomPet2.GetScaleAngle(Type - MODEL_ITEM), 0.0, ObjectSelect.Angle);
...
//		Position[0] += 0.02f;
	...
	{
		Scale *= ItemTRSData.GetItemTRSDataScale(Type - MODEL_ITEM);
	}
