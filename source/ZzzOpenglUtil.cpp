...
float   CameraViewNear      = 100.f;
float   CameraViewFar       = 5000.f;
...
void RenderBitmapScales(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha, float ScaleW, float ScalesH)
{
	if (StartScale)
	{
		x = ConvertX(x);
	...
	if (Scale)
	{
		Width = ConvertX(Width) * ScaleW; // Áp dụng tỷ lệ ScaleFactor
		Height = ConvertY(Height) * ScalesH; // Áp dụng tỷ lệ ScaleFactor
	}
...
void RenderBitmapTrade(int Texture, float x, float y, float Width, float Height, float u, float v, float uWidth, float vHeight, bool Scale, bool StartScale, float Alpha, DWORD color)
{
	...
	{
		glColor4ub(
			static_cast<GLubyte>((color & 0xff)),        // Red
			static_cast<GLubyte>((color >> 8) & 0xff),   // Green
			static_cast<GLubyte>((color >> 16) & 0xff),  // Blue
			static_cast<GLubyte>((color >> 24) & 0xff)   // Alpha
		);
	...
	glColor4f(1.f, 1.f, 1.f, 1.f); // Reset lại màu sắc về mặc định
	glEnd();
}
