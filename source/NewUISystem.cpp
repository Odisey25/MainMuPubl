...
void SEASON3B::CNewUISystem::Show(DWORD dwKey)
{
	...
	if(m_pNewUIMng) 
	{
		...
		else if (dwKey == INTERFACE_ExpandWarehouse)
		{
			...
			g_pStorageInventoryExt->SetPos(GetWindowsX() - 190 * 3, 0);
...
void SEASON3B::CNewUISystem::RenderItem3DFree(float sx, float sy, float Width, float Height, int Type, int Level, int Option1, int ExtOption, bool PickUp, float Scale)
{
	...
	if (m_Resolution >= 3) { SizeItem += 0.30f; }
