...
{
	if (gCustomMonsterGlow.CheckCustomByMonster(pObject->Type) == true)
	{
		pModel->BeginRender(1.f);
		pModel->StreamMesh = 1;
		for (int j = 0; j < 8; j++)
		{
			pModel->RenderMesh(j, RENDER_TEXTURE, pObject->Alpha, pObject->BlendMesh, pObject->BlendMeshLight, pObject->BlendMeshTexCoordU, pObject->BlendMeshTexCoordV);
	...
	{
		if ((MODEL_MONSTER01 + it->MonsterID) == pObject->Type)
		{
			//*(float*)(a2 + 72) = (float)(it->ColorR / 255.0f);
			//*(float*)(a2 + 76) = (float)(it->ColorG / 255.0f);
			//*(float*)(a2 + 80) = (float)(it->ColorB / 255.0f);
			pModel->BodyLight[0] = (float)(it->ColorR / 255.0f);
			pModel->BodyLight[1] = (float)(it->ColorG / 255.0f);
			pModel->BodyLight[2] = (float)(it->ColorB / 255.0f);
			pObject->BlendMeshLight = (float)(it->ColorR / 255.0f);
			pObject->BlendMeshTexCoordU = (float)(it->ColorG / 255.0f);
			pObject->BlendMeshTexCoordV = (float)(it->ColorB / 255.0f);

			pModel->RenderMesh(it->EffectIndex, it->EffectType, pObject->Alpha, pObject->BlendMesh, pObject->BlendMeshLight, pObject->BlendMeshTexCoordU, pObject->BlendMeshTexCoordV);
			pModel->StreamMesh = -1;
		}
	}
