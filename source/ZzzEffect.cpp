...
bool AddRangeAttack(vec3_t Position, float Range, short PKKey, int DamageKey[], int* piCount, int iMaxKey) // Sửa Lỗi 2
{
	if (*piCount >= iMaxKey)
	{
		return false;
	...
	for (int i = 0; i < MAX_CHARACTERS_CLIENT; i++)
	{
			...
			&& (o->Kind == KIND_MONSTER || (o->Kind == KIND_PLAYER && (c->Key == PKKey || DamageChr))))
		{
			if (Skill == AT_SKILL_STORM
				|| Skill == AT_SKILL_EVIL
				|| Skill == MASTER_SKILL_ADD_EVIL_SPIRIT_IMPROVED1
				|| Skill == MASTER_SKILL_ADD_EVIL_SPIRIT_IMPROVED2

				|| Skill == MASTER_SKILL_ADD_EVIL_SPIRIT_IMPROVED1 || Skill == MASTER_SKILL_ADD_EVIL_SPIRIT_IMPROVED2 || (AT_SKILL_EVIL_SPIRIT_UP <= Skill && AT_SKILL_EVIL_SPIRIT_UP + 4 >= Skill) || (AT_SKILL_EVIL_SPIRIT_UP_M <= Skill && AT_SKILL_EVIL_SPIRIT_UP_M + 4 >= Skill)
				)
			{
	...
	case BITMAP_LIGHT_RED:
	{
				...
				|| g_isCharacterBuff(o->Owner, eBuff_Def_up_Ourforces)))
		{
			...
			if (g_isCharacterBuff(o->Owner, eBuff_Def_up_Ourforces)
				|| g_isCharacterBuff(o->Owner, EFFECT_GREATER_DEFENSE_SUCCESS_RATE_IMPROVED)
				|| g_isCharacterBuff(o->Owner, EFFECT_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED))
			{
				if (rand() % 70 == 7)
				{
