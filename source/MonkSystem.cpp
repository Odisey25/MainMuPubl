...
{
	if (_Type != MODEL_HELPER + 37 && gCustomPet2.GetInfoPetType(Hero->Helper.Type - 1171) != 5 && _Type != MODEL_HELPER + 2 && _Type != MODEL_HELPER + 3)
		return false;
...
bool CMonkSystem::RageCreateEffect(OBJECT* _pObj, int _nSkill)
{
	...
	case AT_SKILL_ATT_UP_OURFORCES:
	{
		...
		vec3_t vLight;
		Vector(0.2f, 0.4f, 1.2f, vLight); 

		vec3_t _vAngle;
		VectorCopy(_pObj->Angle, _vAngle);

		CreateEffect(BITMAP_SWORD_EFFECT_MONO, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 5, 5, 5, 2.0f);

		for (int i = 0; i < 3; ++i)
		{
			Vector(0.6f, 0.65f, 1.2f, vLight);
			CreateEffect(MODEL_SHOCKWAVE_GROUND01, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 5, 5, 5, 1.5f - (i * 0.1f));

			Vector(0.65f, 0.8f, 1.2f, vLight);
			CreateEffect(BITMAP_EVENT_CLOUD, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 5, 5, 5, 1.5f);
		}

		Vector(0.5f, 0.55f, 1.2f, vLight);
		CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 5, 5, 5, 2.0f);

		Vector(0.5f, 0.55f, 1.0f, vLight);
		CreateEffect(BITMAP_MAGIC, _pObj->Position, _pObj->Angle, vLight, 13, _pObj, -1, 5, 5, 5, 1.5f);
	}
	return true;
	...
	case MASTER_SKILL_ADD_FITNESS_IMPROVED:
	{
		...
		CreateEffect(BITMAP_SWORD_EFFECT_MONO, _pObj->Position, _pObj->Angle, vLight, 1, _pObj, -1, 5, 5, 5, 2.0f);
		...
		CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle, vLight, 4, _pObj, -1, 5, 5, 5, 2.0f);
	...
	case MASTER_SKILL_ADD_GREATER_DEFENSE_SUCCESS_RATE_ENHANCED:
	{
		...
		// ⚡ Điều chỉnh kích thước và độ lan tỏa của hiệu ứng chính
		CreateEffect(BITMAP_SWORD_EFFECT_MONO, _pObj->Position, _pObj->Angle, vLight, 2, _pObj, -1, 5, 5, 5, 2.0f);
		...
		CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle, vLight, 5, _pObj, -1, 5, 5, 5, 2.0f);
	...
	case MASTER_SKILL_ADD_DRAGON_LORE_ENHANCED:
	{
		...
		vec3_t vLight;
		Vector(1.0f, 1.0f, 1.0f, vLight);
		CreateEffect(BITMAP_LAVA, _pObj->Position, _pObj->Angle, vLight, 0, _pObj, -1, 5, 5, 5, 2.0f);
	...
	case AT_SKILL_MUITENPHUONGHOANG:
	{
		...
		//CreateEffect(MODEL_phoenix_shot_effect, _pObj->Position, _pObj->Angle, vLight, 0, to);
		//CreateEffect(MODEL_phoenix_shot_effect, _pObj->Position, _pObj->Angle, vLight, 1, to);
		//CreateEffect(MODEL_phoenix_shot_effect, _pObj->Position, _pObj->Angle, vLight, 2, to);
		//InitLower();
		PlayBuffer(SOUND_RAGESKILL_DARKSIDE);
	}
	break;

	case AT_SKILL_DARKSIDE: // Ảo Ảnh Liên Hoàn RF
	case MASTER_SKILL_ADD_DARK_SIDE_IMPROVED:
	case MASTER_SKILL_ADD_DARK_SIDE_ENHANCED:
	{
		vec3_t vLight;
		
		// Hiển thị hiệu ứng Dark Side trên quái trước khi reset dữ liệu
		for (int i = 0; i < 5; ++i)
		{
			vec3_t attackPos;
			attackPos[0] = _pObj->Position[0] + (rand() % 100 - 50) * 0.1f;
			attackPos[1] = _pObj->Position[1] + (rand() % 100 - 50) * 0.1f;
			attackPos[2] = _pObj->Position[2] + (rand() % 50 - 25) * 0.1f;

			Vector(1.0f, 0.6f, 0.6f, vLight);
			CreateEffect(BITMAP_EVENT_CLOUD, attackPos, _pObj->Angle, vLight, 1, _pObj, -1, 0, 0, 0, 5.0f);
		}

		// Các hiệu ứng chính của Dark Side
		for (int i = 0; i < 3; ++i)
		{
			Vector(1.0f, 0.4f, 0.0f, vLight);
			CreateEffect(BITMAP_MAGIC, _pObj->Position, _pObj->Angle, vLight, 14, NULL, -1, 0, 0, 0, 10.0f);
		}

		for (int i = 0; i < 3; ++i)
		{
			Vector(1.0f, 0.5f, 0.2f, vLight);
			CreateEffect(MODEL_WINDFOCE, _pObj->Position, _pObj->Angle, vLight, 2, _pObj, -1, 0, 0, 0, 10.0f);
		}

		// Phát âm thanh kỹ năng
		PlayBuffer(SOUND_RAGESKILL_DARKSIDE);

		// Reset dữ liệu quái bị đánh sau khi tạo hiệu ứng xong
		InitDummyCal();
	}
	break;
