...
void CLoginWin::Create()
{
	...
	if (m_SavePassOnOff)
	{
		m_pPassInputBox->SetText(m_Pass);
	}
...
void CLoginWin::RenderControls()
{
	...
	if (g_pBCustomMenuInfo->RenderCheckBoxMini(int((CWin::GetXPos() + 124) / g_fScreenRate_x),
		int((CWin::GetYPos() + 158) / g_fScreenRate_y), 0xFFFFFFFF, m_SavePassOnOff == 1 ? TRUE : FALSE, "Save password"))
	{
		m_SavePassOnOff ^= 1;
...
void CLoginWin::RequestLogin()
{
	...
	else
	{
		if (CurrentProtocolState == RECEIVE_JOIN_SERVER_SUCCESS)
		{
			...
			regkey.WriteDword("SavePass", m_SavePassOnOff);
			if (m_SavePassOnOff)
			{
