...
//	gLoadData.AccessModel(MODEL_HELPER+132, "Data\\Item\\", "goldenmaple_cham");
	...
	{
		LoadItemModel((it->second.ItemIndex + MODEL_ITEM), ((it->second.ItemIndex >= BConverITEM(7, 0) && it->second.ItemIndex < BConverITEM(12, 0)) ? gProtect.m_MainInfo.PathSet : gProtect.m_MainInfo.PathItem), it->second.ModelName, -1);
	}
	...
	for (int n = 0; n < MAX_CUSTOM_WING; n++)
	{
		...
		{
			LoadItemModel((gCustomWing.m_CustomWingInfo[n].ItemIndex + MODEL_ITEM), gProtect.m_MainInfo.PathWing, gCustomWing.m_CustomWingInfo[n].ModelName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		...
		{
			LoadItemModel(it->second.ItemIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.ModelName, -1);
			LoadItemModel(it->second.TextureLeftIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.TextureLeftName, -1);
			LoadItemModel(it->second.TextureRightIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.TextureRightName, -1);
		}
	}

	for (std::map<int, CUSTOM_RING_INFO>::iterator it = gCustomRingPen.m_CustomRingPenInfo.begin(); it != gCustomRingPen.m_CustomRingPenInfo.end(); it++)
	{
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			LoadItemModel((it->second.ItemIndex + MODEL_ITEM), gProtect.m_MainInfo.PathBow, it->second.ModelName, -1);
		}
	}
	for (std::map<int, CUSTOM_PET_INFO>::iterator it = gCustomPet2.m_CustomPetInfo.begin(); it != gCustomPet2.m_CustomPetInfo.end(); it++)
	{
		//g_ConsoleDebug->Write(1, "Load Item Pet %d %d [%s]", (int)it->second.ItemIndex, (it->second.ItemIndex + MODEL_ITEM), it->second.ModelName);
		LoadItemModel((it->second.ItemIndex + MODEL_ITEM),gProtect.m_MainInfo.PathPet, it->second.ModelName, -1);
	}
}
...
//---------------------------------------------------------------------------------
	...
	::LoadBitmap("Item\\ArchangelusWeapon_render.jpg", eArchangelusWeapon, 9729, 10497, 1, 0);
	::LoadBitmap("Effect\\pin_star.jpg", ePinStar, 9729, 10497, 1, 0);
	::LoadBitmap("Item\\PhoenixSoul_render.JPG", ePhoneix, 9729, 10497, 1, 0);
	//==Ss6
	...
	for (std::map<int, CUSTOM_ITEM_INFO>::iterator it = gCustomItem.m_CustomItemInfo.begin(); it != gCustomItem.m_CustomItemInfo.end(); it++)
	{
		LoadItemTexture((it->second.ItemIndex + MODEL_ITEM), ((it->second.ItemIndex >= BConverITEM(7, 0) && it->second.ItemIndex < BConverITEM(12, 0)) ? gProtect.m_MainInfo.PathSet : gProtect.m_MainInfo.PathItem), it->second.ModelName, -1);
	}
	...
	for (int n = 0; n < MAX_CUSTOM_WING; n++)
	{
		if (gCustomWing.m_CustomWingInfo[n].Index != -1)
		{
			LoadItemTexture((gCustomWing.m_CustomWingInfo[n].ItemIndex + MODEL_ITEM), gProtect.m_MainInfo.PathWing, gCustomWing.m_CustomWingInfo[n].ModelName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_RF_GLOVES>::iterator it = gCustomBattleGloves.m_CustomGlovesInfo.begin(); it != gCustomBattleGloves.m_CustomGlovesInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			LoadItemTexture(it->second.ItemIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.ModelName, -1);
			LoadItemTexture(it->second.TextureLeftIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.TextureLeftName, -1);
			LoadItemTexture(it->second.TextureRightIndex + MODEL_ITEM, gProtect.m_MainInfo.PathGloves, it->second.TextureRightName, -1);
		}
	}
	...
	for (std::map<int, CUSTOM_BOW_INFO>::iterator it = gCustomBow.m_CustomBowInfo.begin(); it != gCustomBow.m_CustomBowInfo.end(); it++)
	{
		if (it->second.ItemIndex != -1)
		{
			LoadItemTexture((it->second.ItemIndex + MODEL_ITEM), gProtect.m_MainInfo.PathBow, it->second.ModelName, -1);
		}
	}
	for (std::map<int, CUSTOM_PET_INFO>::iterator it = gCustomPet2.m_CustomPetInfo.begin(); it != gCustomPet2.m_CustomPetInfo.end(); it++)
	{
		//Console.AddMessage(1, "%d %s", it->second.ItemIndex, it->second.ModelName);
		LoadItemTexture((it->second.ItemIndex + MODEL_ITEM), gProtect.m_MainInfo.PathPet, it->second.ModelName, -1);
	}
	//==========================================
}
