...
void RenderNotices2()
{
	...
	unsigned int timeInterval2 = 200; // 5 seconds
...
void SetPlayerColor(BYTE PK) // Tên Nhân Vật
{
	switch (PK)
	{
	case 0: g_pRenderText->SetTextColor(255, 215, 0, 255); break; // NPC
	case 1:g_pRenderText->SetTextColor(100, 120, 255, 255); break;
	case 2:g_pRenderText->SetTextColor(140, 180, 255, 255); break;
	case 3:g_pRenderText->SetTextColor(220, 180, 0, 255); break;
...
void RenderBoolean2(int x, int y, CHAT* c)
{
	...
	{
		//g_pRenderText->SetBgColor(60, 100, 0, 150);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Union, RenderBoxSize.cx + 30, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
	}
	if (c->Guild && c->Guild[0])
	{
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Guild, RenderBoxSize.cx + 30, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
	}
	...
	{
		g_pRenderText->SetFont(g_hFontBold);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->ID, RenderBoxSize.cx + 30, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;
		g_pRenderText->SetFont(g_hFont);
	}
	else
	...
	*/
	...
	if (c->LifeTime[1] > 0)
	{
		...
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[1], RenderBoxSize.cx + 30, iLineHeight, RT3_SORT_CENTER);
		RenderPos.y += iLineHeight;

		g_pRenderText->SetTextColor(dwTextColor[0]);
		g_pRenderText->RenderText(RenderPos.x, RenderPos.y, c->Text[0], RenderBoxSize.cx + 30, iLineHeight, RT3_SORT_CENTER);
	}
	else if (c->LifeTime[0] > 0)
	{
		g_pRenderText->SetTextColor(dwTextColor[0]);
...
void AddGuildName(CHAT* c, CHARACTER* Owner) // Tên Guild Ở Đây
{
...
bool CheckAttack_Fenrir(CHARACTER* c)
{
	...
	else if (c->Object.Kind == KIND_PLAYER)
	{
		...
		else if (c->PK >= PVP_MURDERER2 || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && c != Hero) || gInterface.AutoPK == true)
		{
			return true;
		}
...
int	getTargetCharacterKey(CHARACTER* c, int selected)
{
	...
	else if (sc->PK >= PVP_MURDERER2 || (HIBYTE(GetAsyncKeyState(VK_CONTROL)) == 128 && sc != Hero) || gInterface.AutoPK == true)
	{
		return sc->Key;
	}
...
void CloseNPCGMWindow()
{
	...
	if (!g_pUIManager->IsOpen(19))
		return;
	g_pUIManager->Close(19);
}
	...
	if (Success && c->Dead == 0)
	{
		...
		else if (!c->SafeZone || (gMapManager.InBloodCastle() == true) || gMapManager.InChaosCastle() == true)
		{
			if (c->SafeZone &&
				...
				&& !(MASTER_SKILL_ADD_MANA_SHIELD_IMPROVED <= Skill && Skill <= MASTER_SKILL_ADD_MANA_SHIELD_ENHANCED)
				&& !(AT_SKILL_HEAL_UP <= Skill && Skill <= AT_SKILL_HEAL_UP + 4)
				&& !(AT_SKILL_LIFE_UP <= Skill && Skill <= AT_SKILL_LIFE_UP + 4)
				...
				&& gMapManager.InChaosCastle() == false || g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Started == 1)
			{
				if (ClassIndex == CLASS_ELF
					&& (Skill != AT_SKILL_CROSSBOW
						&& Skill != MASTER_SKILL_ADD_TRIPLE_SHOT_IMPROVED
						&& Skill != MASTER_SKILL_ADD_TRIPLE_SHOT_ENHANCED
				...
				else if (Skill == AT_SKILL_BRAND_OF_SKILL)
				{
					...
					Skill == MASTER_SKILL_ADD_SLEEP_IMPROVED
					...
					for (int i = EQUIPMENT_WEAPON_RIGHT; i <= EQUIPMENT_WEAPON_LEFT; i++)
					{
							...
							)
						{
							...
							if (c->Helper.Type != MODEL_HELPER + 2
								...
								&& c->Helper.Type != MODEL_HELPER + 4
								&& gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) != 6
								&& gCustomPet2.GetInfoPetType(c->Helper.Type - 1171) != 5
...
void RenderBooleans()
{
	...
	for (int i = 0; i < MAX_CHAT; i++)
	{
		...
		if (ci->IDLifeTime > 0 || ci->LifeTime[0] > 0)
		{
			...
			if (gProtect.m_MainInfo.NewText == 1)
			{
				RenderBoolean2(ci->x, ci->y, ci);
			}
			else
			{
...
bool g_bHideCursorOnNPCHover = false;
...
void RenderCursor()
{
	...
	if (g_bHideCursorOnNPCHover)
	{
		if (GetAsyncKeyState(VK_TAB) & 1 || GetAsyncKeyState(VK_ESCAPE) & 1) // Kiểm tra khi nhấn TAB hoặc ESC
		{
			g_bHideCursorOnNPCHover = false; // Hiển thị lại con trỏ chuột
		}
		else
		{
			return; // Nếu chưa nhấn TAB hoặc ESC, vẫn giữ nguyên logic ẩn chuột
		}
	}

	else if (g_iKeyPadEnable || ErrorMessage)
	{
		RenderBitmap(BITMAP_CURSOR, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
	}
	else if (SelectedItem != -1)
	{
		RenderBitmap(BITMAP_CURSOR + 3, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
	}
	else if (SelectedNpc != -1)
	{
		if (M38Kanturu2nd::Is_Kanturu2nd())
		{
			RenderBitmap(BITMAP_CURSOR2, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		}
		else
		{
			RenderBitmap(BITMAP_CURSOR + 4, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f, u, v, 0.5f, 0.5f);
		}
	}
	else if (SelectedOperate != -1)
	{
		if ((gMapManager.WorldActive == WD_0LORENCIA && Operates[SelectedOperate].Owner->Type == MODEL_POSE_BOX) ||
			...
			RenderBitmap(BITMAP_CURSOR + 6, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		else
			RenderBitmap(BITMAP_CURSOR + 7, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
	}
	else if ((!Hero->SafeZone) && SelectedCharacter != -1)
	{
		if (CheckAttack() && !MouseOnWindow && g_pNewUISystem->Get_pNewUIMuHelper()->DataAutoMu.Stage == 0)
		{
			...
			{
				RenderBitmap(BITMAP_CURSOR + 2, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
			}
		}
		else
			RenderBitmap(BITMAP_CURSOR, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
	...
	else if (g_pNewUISystem->IsVisible(SEASON3B::INTERFACE_COMMAND))
	{
		if (g_pCommandWindow->GetMouseCursor() == CURSOR_IDSELECT)
		{
			RenderBitmap(BITMAP_INTERFACE_EX + 29, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		}
		...
		{
			RenderBitmap(BITMAP_CURSOR + 1, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		}
	}
		...
		)
	{
		...
		{
			RenderBitmap(BITMAP_CURSOR + 5, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		}
		else
		{
			RenderBitmapRotate(BITMAP_CURSOR + 5, (float)MouseX + 5.f, (float)MouseY + 18.f, 20.f, 20.f, 45.f);
		}
	...
	else if (RepairEnable == 2)
	{
		...
		{
			RenderBitmapRotate(BITMAP_CURSOR + 5, (float)MouseX + 10.f, (float)MouseY + 10.f, 20.f, 20.f, 0.f);
		}
		else
		{
			RenderBitmapRotate(BITMAP_CURSOR + 5, (float)MouseX + 5.f, (float)MouseY + 18.f, 20.f, 20.f, 45.f);
	...
	else
	{
		...
		{
			if (DontMove)
				RenderBitmap(BITMAP_CURSOR + 8, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
			else
				RenderBitmap(BITMAP_CURSOR + 1, (float)MouseX - 2.f, (float)MouseY - 2.f, 20.f, 20.f);
		}
	}
